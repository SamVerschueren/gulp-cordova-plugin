...
test.beforeEach(() => {
	cordova.plugin = sinon.stub().returns(Promise.resolve());
});

test.serial('add plugin', async t => {
	await plugin('foo');

	t.deepEqual(cordova.plugin.lastCall.args, ['add', 'foo', {}]);
});

test.serial('remove trailing / from git-based plugin', async t => {
	await plugin('https://github.com/SamVerschueren/cordova-ua-bb10///');

	t.deepEqual(cordova.plugin.lastCall.args, ['add', 'https://github.com/SamVerschueren/cordova-ua-bb10', {}]);
});

test.serial('add specific version', async t => {
	await plugin('foo', '1.0.0');

	t.deepEqual(cordova.plugin.lastCall.args, ['add', 'foo@1.0.0', {}]);
});

test.serial('add version to git-based plugin', async t => {
	await plugin('https://github.com/SamVerschueren/cordova-ua-bb10', '1.0.0');

	t.deepEqual(cordova.plugin.lastCall.args, ['add', 'https://github.com/SamVerschueren/cordova-ua-bb10#v1.0.0', {}]);
});
...
test.serial('add plugin with variables', async t => {
	...
	t.deepEqual(cordova.plugin.lastCall.args, ['add', 'foo', {cli_variables: {bar: 'bar', baz: 'baz'}}]);					// eslint-disable-line camelcase
});

test.serial('add plugin with version and variables', async t => {
	await plugin('foo', {version: 'latest', variables: {bar: 'bar', baz: 'baz'}});

	t.deepEqual(cordova.plugin.lastCall.args, ['add', 'foo@latest', {cli_variables: {bar: 'bar', baz: 'baz'}}]);			// eslint-disable-line camelcase
});

test.serial('add two plugins when array is provided', async t => {
	await plugin(['foo', 'bar']);

	t.deepEqual(cordova.plugin.args[0], ['add', 'foo', {}]);
	t.deepEqual(cordova.plugin.args[1], ['add', 'bar', {}]);
});
...
test.serial('add two plugins when object is provided', async t => {
	...
	t.deepEqual(cordova.plugin.args[0], ['add', 'foo@latest', {}]);
	t.deepEqual(cordova.plugin.args[1], ['add', 'bar@1.0.0', {}]);
});
...
test.serial('options object in plugin object', async t => {
	...
	t.deepEqual(cordova.plugin.args[0], ['add', 'foo@latest', {}]);
	t.deepEqual(cordova.plugin.args[1], ['add', 'bar@1.0.0', {cli_variables: {baz: 'baz'}}]);								// eslint-disable-line camelcase
});
